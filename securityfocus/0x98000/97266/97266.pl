...
if(!defined $src_host or !defined $src_port or !defined $dst_host or 
!defin=
ed $dst_port)=20
{
=09
=09print "Usage: $0 <source host> <source port> <dest host> <dest 
port>\n";
=09exit;
}=20
else=20
...
=09=09or die $!;
=09my ($packet) =3D makeheaders($src_host, $src_port, $dst_host, 
$dst_port)=
;
...
=09my $tcp_flags =3D $null . $null . $tcp_urg . $tcp_ack . $tcp_psh . 
$tcp_=
rst . $tcp_syn . $tcp_fin ;
=09my $tcp_check =3D 0;
=09my $tcp_header =3D pack('nnNNH2B8nvn' , $src_port , $dst_port , $seq, 
$s=
eq_ack , $tcp_doff_res, $tcp_flags,  $tcp_win , $tcp_check, 
$tcp_urg_ptr);
=09my $tcp_pseudo =3D pack('a4a4CCn' , $src_host, $dst_host, 0, 
$IPPROTO_TC=
...
=09my $ip_header =3D pack('H2CnnB16CCna4a4',=09$ip_ver_len, $ip_tos, 
$ip_to=
t_len, $ip_frag_id,=09$ip_fl_fr , $ip_ttl , $ip_proto , $zero_cksum , 
$src_=
host , $dst_host);
